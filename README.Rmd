...
`ARscore` generates aggregate reactivity scores (ARscores) by comparing the average fold change of a group of peptides to distributions of average fold change from randomly selected peptides, using [`fitdistrplus`](https://cran.r-project.org/web/packages/fitdistrplus/index.html)[^fitdistrplus] and [`limma`](https://bioconductor.org/packages/release/bioc/html/limma.html)[^limma]. To expedite computation and evaluate extreme reactivity scores, random distributions are modeled as gamma distributions with paramaters that change regularly with the number of randomly selected peptides.
